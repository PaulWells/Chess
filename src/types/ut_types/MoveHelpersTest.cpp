...
{
    ChessBoard board = { 0 };
    ChessPiece pawn = ChessPieceHelpers::MakeChessPiece(ChessPieceType::Pawn, false, false, false);
    board[0][0] = pawn;
    Square square = { 0, 0 };
    Vector vector = { 1, 1 };
    Move move = MoveHelpers::CreateMove(board, square, vector);

    Move copyMove = MoveHelpers::CopyMoveWithPromotion(move, ChessPieceType::Knight);

    ChessPiece endState = copyMove.endState;
    uint8_t endStateType = ChessPieceHelpers::GetPieceType(endState);
    test->assert_true(endStateType == ChessPieceType::Knight, "CopyMoveWithPromotion changes piece type of endState");
